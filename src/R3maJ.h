...
inline uint64_t ReadCheckpointTotalTimesteps(const std::string& folder) {
    namespace fs=std::filesystem; std::error_code ec; fs::path root(folder);
    ...
    for (const auto& e:fs::directory_iterator(root,ec)) {
        ...
        try { best=std::max(best,(uint64_t)std::stoull(n)); } catch(...) {}
    }
    return best;
}
...
inline EnvCreateResult EnvCreateFunc(int) {
    ...
    rewards.emplace_back(new GoalReward(-0.8f),r.goal_w); // 20% aggression bias: +10 / -8
    ...
    rewards.emplace_back(new BoostWastePenalty(),r.boost_waste_w);
    ...
    std::vector<TerminalCondition*> terminals={new GoalScoreCondition(),new NoTouchCondition(120*120/8)};
...
inline void StepCallback(Learner*,const std::vector<GameState>& states,Report& report) {
    ...
    double bs=0,bh=0,ba=0,ps=0; uint64_t pc=0;
    for(const auto& s:states){
        bs+=s.ball.vel.Length(); bh+=s.ball.pos.z; ba+=s.ball.angVel.Length();
        for(const auto& p:s.players){ps+=p.vel.Length();++pc;}
    }
    ...
    if(pc) report.AddAvg("player/speed",ps/(double)pc);
}
